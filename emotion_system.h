...
public:
    ...
    void update(float dt, const AudioFeatures& features, float sampleRate) {
        ...
        else if (smoothVolume > 0.3f) {          // 原 0.4 → 0.3，更容易触发 excited
            scoreExcited += 2.0f;
        }
        else {
            scoreJoyful += 2.0f;                   // 中等音量给 joyful 加 2 分（原只加 1 分）
        }

        // 质心（明亮度）
        if (smoothCentroid < 400.0f) {             // 原 500，降低 sad 的阈值并减分
            scoreSad += 1.0f;                       // 原 +2 降为 +1
        }
        else if (smoothCentroid > 1500.0f) {     // 原 2000 → 1500，更容易 excited
            scoreExcited += 2.0f;
        }
        else {
            scoreJoyful += 2.0f;                    // 中等质心给 joyful 加 2 分
        }
        ...
        else if (smoothFlux > 0.15f) {           // 原 0.2 → 0.15，更容易 excited
            scoreExcited += 2.0f;
        }
        else {
            scoreJoyful += 2.0f;                    // 中等通量给 joyful 加 2 分
        }
        ...
        else if (smoothLowHighRatio < 0.8f) {    // 原 0.7 → 0.8，放宽 excited 条件
            scoreExcited += 1.0f;
        }

        // 低频稳定性（低频变化小 → 悲伤，变化大 → 可能激昂）
        if (smoothLowVar < 0.01f) {
            scoreSad += 0.5f;                       // 原 +1 降为 +0.5
        }
        ...
        // 特殊组合：质心高且通量高 → 激昂，质心低且通量低 → 悲伤
        if (smoothCentroid > 1500.0f && smoothFlux > 0.15f) {
            scoreExcited += 2.0f;
        }
